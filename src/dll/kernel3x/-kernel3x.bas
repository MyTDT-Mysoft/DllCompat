...
extern "windows-ms"
  ...
  function InitOnceBeginInitialize ( lpInitOnce as LPINIT_ONCE, dwFlags as DWORD, fPending as long ptr, lpContext as any ptr) as long export
    TRACE("InitOnceBeginInitialize")
    if lpInitOnce = 0 then return 0
    if lpInitOnce = &hB00B1E5 then return false
    *lpInitOnce = &hB00B1E5
    return true
  end function
  
  function InitOnceComplete ( lpInitOnce as LPINIT_ONCE , dwFlags as DWORD , lpContext as any ptr ) as long export
    TRACE("InitOnceComplete")
    return true
  end function
  ...
  function fnCreateFiberEx alias "CreateFiberEx" ( P1 , P2 , P3 , P4 , P5 ) as LPVOID export
    ...
    if (dwFlags and FIBER_FLAG_FLOAT_SWITCH) then
      TRACE( "FIBER_FLAG_FLOAT_SWITCH not supported this may go bad!" )
      dwFlags and= (not (FIBER_FLAG_FLOAT_SWITCH))
    end if
  ...
  function fnCreateFiber alias "CreateFiber" ( P1 , P2 , P3 ) as LPVOID export    
    'TRACE( "" )
    return fnCreateFiberEx( 0 , 0 , 0 , lpStartAddress , lpParameter )
